...
// the strings use no other markdown, and a backtick must never reach the reader.
export function InlineCode({ text }: { text: string }) {
  return (
    <>
      {text.split('`').map((piece, i) =>
        i % 2 === 1 ? (
          <code key={i} className="rounded bg-slate-800 px-1 font-mono text-[0.9em] text-slate-200">
            {piece}
          </code>
        ) : (
          piece
