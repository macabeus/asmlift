...
// `packages/core/src` throws 142 distinct decline messages (the texts reached by
...
// separate NAME to that harvest, so it is listed separately here too). 77 of them classify as "other". Some belong
...
//   frontend/ppc.ts          6  `stwu` with update, a relocation on a stack-pointer adjust, a branch
//                               testing a cr field a call destroyed (no compiler emits it), a return
//                               address `mflr` copied out and the body read as a value, the
...
export const DECLINE_CLASSES: DeclineClass[] = [
  ...
  },
  {
    ...
    //   an unmodelled INSTRUCTION would attribute a 64-bit gap to a decoder that is working. Its
    //   sibling is the PROJECTION of such a value — `structure.ts`'s `hi32` arm refusing a value
    //   that neither renders 64 bits wide nor has an integer type, because `x >> 32` over a
    //   32-bit operand is undefined C. Same capability, one step further down.
    //
  ...
  },
  {
    ...
    // of a temp the guarded body may never assign, a fused guard whose exit edge carries a value the
    // post-loop copies do not reproduce on a zero-trip run, and a post-loop read of a pre-loop value
    // through a loop variable's name that still holds its init on that run. Separate from `loop-shapes`
    ...
    pattern: /post-loop read reaches a temp|do not reproduce on a zero-trip run|its initial value on a zero-trip run/,
  },
  {
  ...
  },
  {
    ...
    // cannot say, AND a frontend that can spend the answer. `FnProto.returns` is the telling; the
    // Thumb frontend reads the pair back, and the PowerPC one refuses a declared wide return
    // rather than reading one half of it, so the same fact closes this on one target and is a
    // loud gap on the other.
    //
