...
function runScript(script: string): Promise<{ code: number; stdout: string; stderr: string }> {
  ...
  return new Promise((res) => {
    ...
    // everything. Since the cache defaults to ON, an inherited environment would run every one of
    // these re-executions — two per selected row, m2c's script and asmlift's, and the run prints
    // the live total on its last line — SERVED off the publishing machine's warm store: the
    // base-versus-head asymmetry the audit exists to bound, in the one gate whose job is to be
    // the reader. (It also keeps a whole-corpus sweep from filling and pruning a developer's
    // shared store.) Cache-off and cache-on-cold produce the same objects, so this is the
    // conservative spelling of the same run; the scripts themselves stay cache-silent, which is
