...
export function cachedM2cResult(inputs: M2cKeyInputs, compute: () => DecompilerResult): DecompilerResult {
  ...
  // v20: a row's diagnostics are picked ERRORS FIRST (core compiler-diagnostics.ts `errorsFirst`),
  //      in the compile modules' messages and in `compilerErrorLines`. Both shape this value's
  //      `errorMarkers` and neither is in a key field, so a v19 entry replays five warnings for a
  //      row whose cause is the error printed below them.
