 ...
 *  left-hand side reads the staging INDEX, so `--base=` compared against whatever happened to be
 *  STAGED and printed a confident line about it. Measured through `bench fan --base=`, which
 *  answered `the artifact at  records no candidate count … the series starts here` off the index.
 *  Every caller passes a ref straight from a `--base` flag, so the guard belongs at the read. */
