...
export const SYNTHETIC: SynthSpec[] = [
  ...
  // `/derived-home` homes a pure value standing on a memory READ. Nothing homes a plain computed
  // value shared by two BRANCH ARMS, or a parameter's narrowing re-spelled at every use — and
  // both are what a bit-test prologue is made of.
  //
  ...
  // No subset closes either row — v(AB) is 58 of 62 and 46 of 48; only all three reach 0. Two
  // earlier derivations of this lattice put 802 at 16.5/41.5/4.0 and 832 at 9.8/36.3/1.8: the ±1
  // is the repair's own SPELLING (where the extension home sits relative to the hoisted zero), so
  // treat the split as ≈17/≈40/4 and ≈11/≈35/2. What is stable across all three derivations is
  // the ordering of the classes, v(AB), and v(C) — and that struct/field recovery is worth ZERO.
  //
  // A FOURTH DEGREE OF FREEDOM the class names hide: the homes' ORDER RELATIVE TO EACH OTHER.
  // Both real rows reach 0 only in one arrangement. On 802 the hoisted `s32 v0 = 0;` must precede
  // `s32 t0 = (s16)a0;` — the other order is byte-identical except that `movs r5, #0` lands two
  // instructions late, and costs 3. On 832 the shared constant must be spelled as a LITERAL inside
  // the hoisted expression with `s32 v0 = 1;` below it; keeping asmlift's shared `v0` and homing
  // it above moves `movs r6, #1` four instructions early and costs 2. So a home lever picks a
  // place, and "the top of the body" is one of the places it can pick wrong.
  //
  // WHAT m2c DOES INSTEAD, and it is one mechanism, not three. m2c binds every register write to a
  // named `Var` (`translate.py:3503-3575` `_eval_once`, the name minted at translate.py:3546), so
  // a value used twice is EMITTED as `temp_rN = …;` and referred to. Its 802 output opens
  // `s16 temp_r1; … temp_r1 = direction;` and then tests `8 & temp_r1` — the entry `lsl/asr` given
  // a name — and carries `var_r5 = 0;` above the chain. asmlift re-DERIVES an expression at each
  // use instead, which is why the same three homes are missing at once.
  //
  // THE ROWS. Every score below is a round-trip: compile the reference with the benchmark's own
  // agbcc, run the CLI on the produced `.s` and `--score-against` the produced `.o`, bundle stamp
  // `asmlift source 5df7ced`, 0 dropped candidates on all seven.
  //
  //   `sxparam` diff:19 — an `s16` parameter tested at three sites. agbcc extends it ONCE at entry
  //   (`lsl r0,#0x10 / asr r2,r0,#0x10`) and every test then reads r2 with the constant in the
  //   other operand; asmlift declares `u32 a0` and re-spells `(s16)a0` at each use, which agbcc
  //   ELIDES (an `& 8` does not care) at the cost of an `add r2,r0,#0` copy per test. FIVE-RUNG
  //   LADDER, all compiled against this row's own target:
  //     19  asmlift today
  ...
  //   The two middle rungs are the correction that matters to whoever builds this: an s32-typed
  //   home ALONE buys 3 points, not 17 — the capability is a home that CARRIES THE WIDTH, or an
  //   s32 home plus the cast-cleanup that a width-carrying home makes automatic. (On the real rows
  //   the s32 home is enough, because their re-spellings are not nested inside another cast.)
  //   `zxparam` diff:8 is the unsigned side (`u8`, `lsl #24 / lsr #24`): 8 today, and 0 for the
  //   homed `u8` temp, the homed `s32` temp AND the declared `u8` parameter alike.
  //
  ...
  //   lsl/and` chain once in the entry block, the per-arm spelling emits it in one arm and a
  //   different branch shape in the other. Homing it above the branch scores 0 — but ONLY with the
  //   signed parameter asmlift's winner already declares: spelled `u32`, agbcc constant-folds the
  //   whole `>>31` idiom to `mov r3, #0` and the same repair scores 18. The lever is a home, not a
  //   re-typing, and it must not disturb the signedness the ranked `signed/` candidate picked.
  //
  ...
  //   never wrote. FIRST BLOCKER WATCHED FIRING, not inferred, and then ABLATED: `structure.ts`'s
  //   anchorConstCopies declines any merge variable whose name is claimed by more than one SSA
  //   value (the `nameCount.get(name) !== 1` refusal at structure.ts:1766, documented in that
  //   block's REFUSAL CONDITIONS list). With that one condition ablated and the bundle rebuilt,
  //   this row's ranked line goes from `best signed/defsite: 21` to `best signed/defsite: 0
  //   (match)`. `/defsite` IS enumerated here and wins the row — for the OTHER variable, the
  //   four-way `v`.
  //   On sub_803213C the same pass declines for a different documented reason and it was watched
  //   too: `[anchor] REFUSE v1: opcode=or` / `opcode=and`, 288× each — the merge args there are an
  //   `or` and an `and`, not an unnamed `const`, so `/defsite` cannot reach `armexpr`'s shape at
  //   all. That is why the expression home is a separate row from this one.
  //
  ...
  //   `ldrb r2, [r1]` where the pointer-local spelling keeps `.word 0x3001100` +
  ...
  //   `bx r0` that assigning at first use does not. LADDER: asmlift today (top) 11; NOT hoisting
  //   at all 9; assigning at the first use that needs it 0. The current placement is worse than no
  //   hoist, which is the same signal the real row gives — on sub_802DFC8 the inline spelling
  //   scores 4, a top-assigned base local 8, and a first-use-assigned one 0, and asmlift's own
  //   `/livebase` candidate (which does emit the base local, at the top) scores 66 against the
  ...
  //   enclosing scope here IS the function body. That file's header records this placement
  //   question as unmeasured; this pair is the measurement.
  //
  // THE CONTROL — ONE row, and it is the only one in this family that was measured to move under
  // the fix it brackets:
  //   `armkeep` MATCH — the same pure expression computed in BOTH arms, but consumed inside each
  //   arm rather than merged out of the `if`. agbcc keeps both copies, asmlift emits both copies,
  //   and the row matches. Hoisting it above the branch scores 9 against this row's own target, so
  //   a cross-arm home keyed on "the same pure expression appears in both arms" BREAKS it and the
  //   row says so. What it does NOT bracket: a home keyed on a MERGE VARIABLE's incoming values.
  //   Nothing can, on this compiler — measured, `if (a) { out[0]=0; m=(b<<3)+7; } else { out[0]=1;
  //   m=(b<<3)+7; } out[1]=m;` is hoisted by AGBCC ITSELF into one pre-branch copy, so the ROM
  //   never carries the per-arm spelling for that shape and no reference source can produce it.
  //
  // THREE CONTROLS THIS FAMILY DELETED, each with the measurement that killed it. They are named
  // here so the next round does not re-derive them:
  //   `sxwide` (`void sxwide(s16 d, s32 *out){ out[0] = d; out[1] = d + 1; }`) was declared the
  //   bracket for the width-carrying home. All FOUR spellings — asmlift's `u32` + per-use
  //   `(s16)a0`, an `s16` home, an `s32` home, and a declared `s16` parameter — score 0 against
  //   its own target, byte-identical. Where the uses consume the FULL value agbcc CSEs the
  //   extension, so per-use and homed are the same program; the divergence exists only where the
  //   narrowing is ELIDABLE (masked uses). The row could not have detected the fix firing, or
  //   over-firing, in either direction.
  //   `maskone` (`s32 m = 0; if (d & 2) m = 0x400;`) was declared the single-claimant half of the
  //   `nameCount !== 1` pair. agbcc if-converts its one-armed `if`, so there is no merge block
  //   with params and anchorConstCopies is never consulted: its ranked line is `2 candidate(s)
  //   scored … best signed: 0 (match)` with NO `/defsite` candidate at all, and it is unchanged
  //   with the refusal ablated. Zero claimants, not one.
  //   `basefirst` (the three-offset base whose first use IS the first statement) was declared the
  //   bracket for both base levers. Its first use being its first statement makes top-placement
  //   and first-use placement the same tree, and its base has 3 uses so `single-use` never applies:
  //   it is unchanged under each ablation and under both together.
  //
  // WHAT ALREADY GATES THESE LEVERS (Phase 5, by ABLATION over the whole corpus — the measure is
  // whether a ROW moves, not whether a test exists). Each ablation below was applied to the
  // packages/ tree, the bundle rebuilt, the tier re-run, and reverted:
  //   The `maskchain` lever (`nameCount !== 1` off) moves 19 rows and regresses NONE of the 863.
  //   Synthetic (13): `mergeloop` agbcc 10→MATCH and mwcc_242_81 18→MATCH and gcc2.7.2kmc 18→7,
  //   `maskchain` 21→MATCH, `breakloop` kmc 11→5 and mwcc 15→14, `structarr` agbcc 5→2 and mwcc
  //   45→44, `nestedloop` kmc 16→13, `countdown` mwcc 23→18, `clampu8` kmc 6→5, `arraysum` mwcc
  //   45→44, `powi` mwcc 32→31. Real (6): `sub_802DFC8` 62→23, `CalcCRC16` 38→23,
  //   `TrySetCantSelectMoveBattleScript` 192→146, `VramGetTotalAllocatedTiles` 18→7,
  //   `CountCollectedGems` 328→324, `ConfigureEntityBehavior` 266→265. So the pre-existing
  //   `mergeloop` rows (tagged `merge-chain`) ALREADY gate this capability and are the stronger
  //   gate — two of them go all the way to MATCH where `maskchain` is the non-loop isolate closest
  //   to the real rows. And nothing anywhere brackets an over-fire: zero rows regress. That is not
  //   permission to delete the refusal. Its stated rationale is that a shared name has readers and
  //   writers between the def site and the edge — a MEANING concern, which a score cannot referee.
  //   The lever is "refine the refusal", and 0-regressions bounds only its placement cost.
  //   The `basecell` lever (`single-use` gate off) moves 6 rows and REGRESSES TWO: `basecell`
  //   2→MATCH, `readcall` 6→17, `bg_mix` ido7.1 1→10, `dma_fill_uninit` 68→71, and — the brackets
  //   — `readarm` MATCH→8 and `armshare` MATCH→17, both pre-existing rows tagged `read-once`.
  //   The `basehome` lever (init placed at first use instead of `[...inits, ...rest]`) moves 4
  //   rows and REGRESSES TWO: `basehome` 11→MATCH, `sizebound` 16→20, and — the brackets —
  //   `mixpoll` MATCH→2 and `onepoll` MATCH→2, both pre-existing rows tagged `value-home`.
  //   The two base levers INTERACT: with both ablated, `armshare` and `readcall` come back and
  //   only `readarm`, `mixpoll` and `onepoll` are still lost, while `basecell` and `basehome` are
  //   both MATCH. Whoever ships them should ship them together and re-measure, not one at a time.
  ...
  //   `narrow`, both diff:4) reach 0 with the homed spelling AND with the declared narrow
  //   parameter. Expect those two to flip WITH this family's rows, as confirmation and not as
  //   collateral. No over-fire bracket for the width home exists in the corpus and this family
  //   could not author one (see `sxwide` above); what would earn one is a shape where the home
  //   forces a SPILL that the per-use re-derivation avoids — unbuilt.
  //
  // WHERE A FIX WOULD GO, and what it collides with. `packages/core/src/structure/structure.ts`
  // (anchorConstCopies' single-claimant refusal, ~1766), `packages/core/src/l3/basecse.ts` (the
  // `single-use` gate at :154 and the `[...inits, ...rest]` placement at :290), and
  // `packages/core/src/structure/analysis.ts` + `packages/core/src/rank.ts` for the home itself —
  // the three existing axes there are ONE capability gated on three incidental shapes (an address
  // at 2+ sites, a value outside a LOOP, a value standing on a READ), and this family's rows are
  // exactly the case none of the three admits. Two of those files, `structure.ts` and `rank.ts`,
  // are also on the file list of the per-site-signedness round that landed as 5df7ced — in
  // DISJOINT REGIONS (that round edits ~577, ~1976-2050 and ~2221; the refusal sits at ~1766), and
  // disjoint by MEASUREMENT rather than by argument: with that round's `packages/core/src` in the
  // tree these seven rows and both real rows score exactly what they score without it
  // (19 / 8 / 35 / MATCH / 21 / 2 / 11, and 62 / 48). Nothing here needs `l3/typing.ts`,
  // `backend/cfamily.ts` or `rank.ts`'s `SIGN_CANDS`: every `signed/` candidate scores identically
  // to its `unsigned/` twin on all 24 of 802's and all 4 of 832's, so the signedness axis
  // contributes zero on both real rows.
  //
  // NOTHING IN THIS FAMILY DECLINES. All seven rows produce a scorable candidate on agbcc, so no
  // score here is a stand-in for a frontend link.
  //
  // NO NEW TAG. The first attribution of these two real rows proposed one — `param-width`, on the
  // theory that asmlift must consume `FnProto.params`' typed list (`proto.ts` accepts
  // `ParamType[]` and `protoArity` reads only its length). Measured, that theory does not hold up
  // as a separate family: handing asmlift the exact declared types via
  // `--proto '{"sub_802DFC8":{"params":["s16","void *"],"returnsVoid":true}}'` returns
  // `24 candidate(s) scored, 24 dropped, best unsigned/setup-args/defsite: 62` — the same ranked
  // line as the row's own `--proto`, with byte-identical stdout — while homing the cast, with no
  // declared type anywhere, closes both real rows. The rows carry that typed list anyway, so both
  // decompilers hold the same facts (m2c is handed the function's own prototype by the harness)
  // and so the rows start measuring the day the types are consumed.
  //
  ...
  // on an `s32 *` it has no struct for; that is its documented raw-pointer member rendering, not
  // context withheld — the `ctx` declares the full prototype, parameter names and all. (2) Two
  // (`basecell`, `basehome`) type the address constant as `void *` and read members off it
  // (`(void *)0x03001100->unk3`), the same behaviour every raw-address row in this file already
  // records; on `basecell` that is the ONLY cause — its single store through the parameter is
  // rendered correctly as `*out`. What the output SHOWS is the point: on `armexpr` m2c hoists the
  ...
  // above the chain, and on `zxparam` it opens with `u8 temp_r0 = a;`. Every home this family is
  // about, m2c has — and on `armkeep`, so does the restraint not to use one.
  //
