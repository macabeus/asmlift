...
| **MIPS**           | Nintendo 64      | IDO 7.1 (SGI's compiler, run via [ido-static-recomp](https://github.com/decompals/ido-static-recomp)), KMC GCC and GCC 2.7.2 | MIPS has **delay slots**: the instruction _after_ a branch executes _before_ the branch takes effect. The frontend has to model that faithfully                                                                       |
...
| **Dropped / withheld / not compiled** | Dropped: the scorer refused the candidate. Withheld: it compiled and scored, but was refused publication for want of a byte-exact proof. Not compiled: never handed to the scorer, because its fan was **stillborn** — the default candidate and one probe per variation were all rejected for the same reason (`packages/core/src/stillborn.ts`) |
...
| **Toolchain**                         | The exact vintage compiler+assembler that is the spec (agbcc, IDO, KMC GCC, GCC 2.7.2, mwcc)                                                                                                                                                                                                                                                      |
| **Target**                            | asmlift's ID for one ISA+toolchain pair (`agbcc`, `ido7.1`, `gcc2.7.2kmc`, `gcc2.7.2`, `mwcc_242_81`, `mwcc_233_163n`, `mwcc_247_107`); "target object" = the original object a candidate is byte-compared against                                                                                                                                |
