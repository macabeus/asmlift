...
After extending named-home tracing, all 30 declaration-study sources were recompiled real/off/on and their assembly compared identical.
...
| `gcc/stmt.c:3312` excludes volatile/addressable locals from pseudos                                                            | Whether a candidate is deliberately forcing memory and whether the observed target home permits it | Candidate-local declaration policy belongs in L3; `packages/core/src/structure/structure.ts:4582` begins local/slot construction. Address-taking is not a way to choose a register home.                                                                                                                                                            |
...
next local label or ignoring ELF symbol size. GNU alias spellings are normalized; numeric tokens
