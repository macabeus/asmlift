...
ordering, loop structure and literal-pool data. None licenses a new score-improving change.
...
not a new priced change: high-register homes can require extra low-register operations. No new
...
interventions and _neutral_ for others. They do not count zero generated candidates for a new spelling.
No new _no reach_ or _does not compose_ claim is made. In particular, non-additive scores alone
do not show that Cartesian dimensions cannot enumerate a combination. A full Cartesian product can
reach it if both transformations are admitted and compose; missing candidates require separate evidence.
...
- [Declaration curve and observed homes](lbg-attribution-declarations.md): two published-cell spot
  checks, seven perturbations across four basin/FAKE conditions, and real/trace-off/trace-on neutrality.
