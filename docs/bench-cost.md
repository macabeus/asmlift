...
- **`pnpm bench baseline <sym>`** prints the row as published _plus its price_:
  ...
  ```
  kleod:WorldMapScreenCheckNewWorldUnlocked:agbcc  asmlift=nonmatch 106/361  m2c=noncompile -/-  fan=3600 rank=103.3s
  ```
...
Summed out of the committed artifact of 2026-09-19: the ranked pass alone is **1,026 s over 180
real rows** and **1,223 s over 715 synthetic rows**; wall clock is lower because eight shards run in
parallel — that run walled 288.1 s and 280.3 s.
...
The single row `kleod:PauseMenuScreenHandler:agbcc` is 230 s of that real total — **22% of the tier
