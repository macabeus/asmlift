...
- **`pnpm bench baseline <sym>`** prints the row as published _plus its price_:
  ...
  ```
  kleod:WorldMapScreenCheckNewWorldUnlocked:agbcc  asmlift=nonmatch 106/361  m2c=noncompile -/-  fan=3600 rank=87.3s
  ```
...
Summed out of the committed artifact of 2026-09-14: the ranked pass alone is **687 s over 152
real rows** and **373 s over 675 synthetic rows**; wall clock is lower because eight shards run in
parallel. The single row `kleod:PauseMenuScreenHandler:agbcc` is 191 s of that real
total — **28% of the tier in one row.**
