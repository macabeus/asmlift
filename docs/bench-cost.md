...
| `npx vitest run` (root config)           | **~33 s** — 231 files, 4,217 tests, 0 skipped; both counts grow as tests are added               | `npx vitest run` on this branch, 2026-09-12, in a shell that had sourced `/tmp/wt-env.sh`. The pre-remediation row said 230 / 4,199, which was neither state's count — the branch writing the sentence invalidated it in the same commit. Without that `source`, two `candcache-namespace.test.ts` tests SKIP (`skipIf(!HAVE_AGBCC)`), so a run reporting skips means your shell lacks the toolchains, not that the suite shrank |
