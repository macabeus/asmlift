...
| `pnpm bench sweep`                       | **~60 s** warm over all 1,062 rows, both arms — nothing is compiled; **~177 s** with every target built | `time pnpm bench sweep` → 59.0 s / 61.8 s / 62.8 s across three runs on two worktrees, 2,124 records, 2026-09-12 (the last of them after the input-digest fields, which cost ~2 s); `ASMLIFT_BENCH_CACHE=0` → 176.9 s, same day. `run/sweep.ts`'s header table used to say 28.8 s here — that was a standalone probe's build+lift loop, not the command                                                                                                                                                                                                                     |
...
| `pnpm bench sweep --base <ref>`          | **~232 s** whole-corpus lift-only, both sides, including provisioning a cold base worktree              | `time pnpm bench sweep --base 910fd416` → 3 min 52 s (this tree 59.4 s, base tree 170.1 s — its first target builds are cold), 2026-09-12                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `pnpm bench gates --pass <pass>`         | **~29 s**                                                                                               | `time pnpm bench gates --pass unmerge` → 29.3 s over 309 rows, 2026-09-12                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `npx vitest run` (root config)           | **~37 s** — 232 files, 4,254 tests, 0 skipped; both counts grow as tests are added                      | `npx vitest run` on `tooling/bench-sweep`, 2026-09-12, in a shell that had sourced `/tmp/wt-env.sh`: 36.35 s. The row before this one said 232 / 4,242 at 39 s (this branch's own earlier commit), before that 231 / 4,223 at 33 s and 230 / 4,199 — each branch that adds a test invalidates the row in the same commit, which is why the row is re-measured rather than inherited. Without that `source`, two `candcache-namespace.test.ts` tests SKIP (`skipIf(!HAVE_AGBCC)`), so a run reporting skips means your shell lacks the toolchains, not that the suite shrank |
...
main && echo clean`.** An empty selection — a typo'd `--only`/`--project`/`--asm-dir` — exits 2
  ...
  **How far back `--base` reaches: `85f81116` (2026-09-09).** The driver loads nine harness modules
  ...
  for (3.1 s, measured against `2bb1cde6`); what the check cannot see is a module that still exists
