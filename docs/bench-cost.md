...
- **`pnpm bench baseline <sym>`** prints the row as published _plus its price_:
  ...
  ```
  kleod:WorldMapScreenCheckNewWorldUnlocked:agbcc  asmlift=nonmatch 106/361  m2c=noncompile -/-  fan=3600 rank=208.2s
  ```
...
Summed out of the committed artifact of **2026-09-25**, taken at `cf2d4eee` (`match/fadd-fpu`
on #261: single-precision hardware float on MIPS and PowerPC, 13 rows gained): the ranked pass
alone is **1,842 s over 195 real rows** and **774 s over 795 synthetic rows**; wall clock was
508.6 s and 312.8 s on tiers that overlap, and **508.7 s** end to end (the CLI's own `Done in`
line). Against `origin/main` it is **113 field changes over 18 rows**, fan **69,204 → 69,204
(1.00×)** over 977 comparable rows, while its ranked pass reads **3,219.5 s → 2,613.1 s (0.81×)**,
and 13 rows (3.4 s) rank here that declined at `origin/main`. The dearest single row is **227 s**
on `kleod:PauseMenuScreenHandler:agbcc`, **12% of the tier** on its own, a row this branch does not
touch.
...
**248 s** on `marioparty4:SceneMain:mwcc_242_81`, 12% of the tier — the same row reads 148 s here
with its fan unmoved. The same fan priced 0.81× one artifact later: the seconds are the box, not
the branch.

The artifact before that, taken 2026-09-24 at `b7857a14`
