...
| `pnpm bench fan <huge row>`, REFUSED     | **the whole enumeration** — ~33 min at LBG scale, and only then the refusal                                          | `FAN_SCORE_LIMIT` is tested on `cands.length` at `apps/benchmark/src/run/fan.ts:951`, i.e. after the pre-count enumeration; read at `3a4fd60f` on 2026-09-12. The 2,000 guard bounds what gets COMPILED, never what gets enumerated, so it is not a cheap shield                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
