...
| ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `pnpm bench run` (all tiers)             | **~793 s ≈ 13 min**, over 1,200 rows                                                                                 | the two tier lines of the 2026-09-21 ship run at `06021f09`: `✓ synthetic: 822 results in 295.1s`, `✓ real: 378 results in 497.6s`, 0 SKIPs. Docker up, candidate cache on and PART-COLD — this branch's own commit re-mints the namespace, so some rows compiled cold inside it                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `pnpm bench run --tier synthetic`        | **~295 s** over 822 rows                                                                                             | same run, 2026-09-21. The row this replaces read 181 s over 812 rows on 2026-09-13; ten more rows do not account for the difference, the cold part of the cache does                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `pnpm bench run --tier real`             | **~498 s** over 378 rows, with the stillborn stop                                                                    | same run, 2026-09-21. Every earlier figure in this cell priced a tier that compiled every candidate of every fan — 2,170 s on 2026-09-13 over 252 rows, and 2,941–5,720 s cold or contended in this repo's run logs of 2026-09-05/07/08. That is a different amount of work, not a slower machine                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
- **`pnpm bench baseline <sym>`** prints the row as published _plus its price_:
  ...
  ```
  kleod:WorldMapScreenCheckNewWorldUnlocked:agbcc  asmlift=nonmatch 106/361  m2c=noncompile -/-  fan=3600 rank=122.8s
  ```
...
Summed out of the committed artifact of 2026-09-21: the ranked pass alone is **1,188 s over 180
real rows** and **940 s over 717 synthetic rows**; wall clock is lower because eight shards run in
parallel — that run walled 497.6 s and 295.1 s.
The artifact before this one read 1,026 s and 1,223 s over 1,198 rows: `bench diff` across the pair
...
The single row `kleod:PauseMenuScreenHandler:agbcc` is 161 s of that real total — **14% of the tier
