...
- **`pnpm bench baseline <sym>`** prints the row as published _plus its price_:
  ...
  ```
  kleod:WorldMapScreenCheckNewWorldUnlocked:agbcc  asmlift=nonmatch 106/361  m2c=noncompile -/-  fan=3600 rank=92.3s
  ```
...
Summed out of the committed artifact of 2026-09-17: the ranked pass alone is **1,120 s over 165
real rows** and **1,067 s over 678 synthetic rows**; wall clock is lower because eight shards run in
parallel — that run walled 343.3 s and 320.0 s. The 5 new ranked rows are the Mario Party 4 rows
asmlift does not decline, and they are 4.0 s of the real total: the tier's price moved 1,341 s →
1,120 s for reasons that have nothing to do with them, and the synthetic tier's moved 826 s →
1,067 s while nothing about it changed at all. **A figure here is a price under a CACHE STATE and a
MACHINE, not a property of the corpus.** This run's store had again just been pruned past its
4,096 MB cap, and it ran alone on the machine where the artifact it replaces was generated beside
two other worktrees. Earlier artifacts of the same corpus read 818 s and 417 s off a store the day's
benches had filled, 941 s and 889 s off a cold one, and 1,742 s and 1,677 s while several rounds
shared the machine. Read a figure beside the cache state AND the load of the run you are planning,
not on its own.

The single row `kleod:PauseMenuScreenHandler:agbcc` is 222 s of that real total — **20% of the tier
