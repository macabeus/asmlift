...
| `npx vitest run` (root config)           | **~39 s** — 232 files, 4,242 tests, 0 skipped; both counts grow as tests are added               | `npx vitest run` on `tooling/bench-sweep`, 2026-09-12, in a shell that had sourced `/tmp/wt-env.sh`: 38.91 s. The row before this one said 231 / 4,223 at 33 s, and the one before that 230 / 4,199 — each branch that adds a test invalidates the row in the same commit, which is why the row is re-measured rather than inherited. Without that `source`, two `candcache-namespace.test.ts` tests SKIP (`skipIf(!HAVE_AGBCC)`), so a run reporting skips means your shell lacks the toolchains, not that the suite shrank |
