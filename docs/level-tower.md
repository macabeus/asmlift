...
| frontend | a float argument at an entry block that is a loop header                                                                | its parameters are phis and can be neither completed (`ensureParam`) nor ordered; mwcc lays out the simplest float do-while this way, so it is the next reach limit, and a predecessor-less entry block closes it |
