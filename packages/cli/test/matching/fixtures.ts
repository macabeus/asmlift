...
export const FIXTURES: DecompFixture[] = [
  ...
  },
  {
    ...
    expectSource:
      ...
      // `WriteOrder`), and a destination the pred never wrote sorts first, which is where the
      // def-position proxy puts an outside def too (structure.ts `NO_RECORD`). No instruction
