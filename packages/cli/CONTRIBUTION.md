...
| `src/rank.ts`            | `decompileRanked` — candidates from `@asmlift/core/rank`, scored for real and ranked: types are variations the differ ranks, not guesses                                                                                                                                                                                                                                                                                                                                         |
| `src/report.ts`          | `decompileWithReport` — the core `TraceReport` enriched with scores, per-pattern deltas, ranked candidates                                                                                                                                                                                                                                                                                                                                                                       |

The CodeWarrior Docker image (`asmlift-ppc:latest`) is built from
