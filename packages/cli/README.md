...
| `--proto`         | Function prototypes, inline JSON or a path to it (`{"sym": {"params": N \| ["u8", ...], "returns": "long long", "returnsVoid": true}, ...}`): a callee's `params` says how many ARGUMENT REGISTERS the call occupies — a bare count states that number directly, a TYPED list states each C parameter's type and is summed (a `long long` is one parameter and two registers) — `returns` states the type it hands BACK, which is what gets the callee declared in the candidate's own source (a typed `params` list is required for that, and every spelling must be one the candidate's prelude declares — `int64_t` and a project typedef alike are silence, not a guess) and, on Thumb, how a 64-bit return is known to come home in a register pair; the PowerPC frontend does not build that pair and REFUSES a return declared wider than a register rather than reading one half of it — and the decompiled function's OWN entry gives its parameter widths (below) and its void-ness. `"returnsVoid": true` and `"returns": "void"` are the same fact and a table stating both differently is refused. Every entry is validated — a malformed one is refused (exit `64`), never quietly ignored |
