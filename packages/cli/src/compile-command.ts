...
import { NOT_CACHEABLE, candCache, candidateCacheRefusal, toolchainFileChain } from './candcache';
...
export function compilersFromCommand(template: string, opts: CompileCommandOptions = {}): CommandCompilers {
  ...
  const verdict = (
    ...
    if (status !== 0) {
      return {
        ok: false,
        cmd,
        err: scrub(`compile command failed (exit ${status ?? 'signal'}): ${cmd}\n${output.trim()}`),
      };
    }
  ...
  const stamp = (): string => {
    ...
    // (3b) the compile ENVIRONMENT gcc/cpp read whether or not the template mentions it. The
    // bench's own agbcc pipeline hashes exactly this list (apps/benchmark/src/compile/agbcc.ts);
    // the two pipelines compile with the SAME compilers, so answering the question two different
    // ways is how the next hole gets missed. `CPATH` is honoured even under `-nostdinc`.
    for (const v of [
      'CPATH',
      'C_INCLUDE_PATH',
      'CPLUS_INCLUDE_PATH',
  ...
  const cacheable = (content: string): boolean => {
    ...
    if (!saidRefusals.has(reason)) {
      saidRefusals.add(reason);
      process.stderr.write(`[candcache] REFUSED-KEY label=command reason=${reason}\n`);
    }
    return false;
  };
  const viaCache = (content: string, symbol: string, ext: 'c' | 'p', raw: () => Verdict): Verdict => {
    ...
    if (cache.mode !== 'off' && DETERMINISTIC_REJECTION.test(r.err)) {
