...
import { BASECSE_GATES, hoistReusedGlobalBases } from './l3/basecse';
...
export function structureChecked(fn: Fn, opts: Parameters<typeof structure>[1]): SFn {
  ...
  const sfn = hoistReusedGlobalBases(eliminateDeadStores(mergeCommonTails(raw)), BASECSE_GATES, {
    foldsConstAddrOffset: opts?.foldsConstAddrOffset,
  });
