...
//
// A BARE CONSTANT-ARM DIAMOND IS THE EXCEPTION (`SELECT_GATES`), and it is a compiler fact rather
// than a preference. Given `v = K1 … v = K2 … return v` where each arm does NOTHING BUT bind the
// constant, agbcc never emits that diamond back: a constant is unconditionally cheap to materialise,
// so one arm is HOISTED above the compare and the other becomes a conditional skip — `movs r0,#5;
// cmp r1,#0; bne .L; movs r0,#3; .L: bx lr`, four blocks collapsed to two, with no unconditional
// branch to the merge at all. For the {0,1} pair it goes further and folds branchlessly
// (`negs r0,r0; lsrs r0,r0,#31`), erasing the comparison too. Measured on agbcc -O2 -mthumb over
// {0,1}, {1,0}, {5,3} and two 32-bit pool constants (`test/corpus/agbcc-select-*.s`, regenerated by
// `scripts/regen-select-spelling-probes.ts`): in every one the merge-variable spelling loses the
// diamond and the early-return spelling keeps it. So where the TARGET holds that diamond, a merge
...
// and the constant can no longer rise above the compare, so BOTH spellings emit a diamond and they
// differ only in ARM ORDER. Measured, same rig: `int f(int x,int *p){int v; if(x){*p=1;v=5;}
// else{*p=2;v=3;} return v;}` scores 0/byte-exact from the merge variable and 5 from the sunk form,
// and it is the sunk form with the arms FLIPPED that is byte-identical to the merge variable's
// object — i.e. sinking a bodied arm is not wrong, it is the wrong SENSE, and the unranked primary
...
//
// It stops at BARE constants. Whether agbcc hoists a COMPUTED arm is agbcc's own cost question — a
// one-op arm is hoisted (`v = a + b` / `v = b - a` becomes a `subs` above the compare), a three-op
// arm is not — and this pass does not model that threshold, so `constant-arms` refuses one.
//
// WHAT EACH CLAUSE ACTUALLY DECIDES, measured by ablating it and re-lifting all 1039 corpus rows
// offline (`targetAsm` out of the artifact, source-hash diff), on the table as it stands:
//
//     two-arms-one-head        0 rows      a-value-is-returned  0 rows
//     no-arrival-but-the-arms  1 row       `pokeemerald:GetGenderFromSpeciesAndPersonality:agbcc`
//     bare-arms                1 row       `pokeemerald:GiveBerryPowder:agbcc`
//     constant-arms            0 rows ALONE — but the pair, ablated together, decides 8
//
// `constant-arms` IS SUBSUMED ON THIS CORPUS: every row it would decide also has a bodied arm, so
// `bare-arms` gets there first. It is kept because it is the half of the hoist argument about the
// VALUE rather than about the arm, and the two are independent claims — but nothing on the corpus
// stands behind it alone, and a round narrowing this table should know that rather than re-derive
// it. The eight rows the pair decides between them, five of which are MATCH today:
// `marioparty3:UnlinkProcess:gcc2.7.2`, `pokeemerald:GiveBerryPowder:agbcc`,
// `pokeemerald:MathUtil_Div16:agbcc`, `pokeemerald:MathUtil_Div16Shift:agbcc`,
// `sa3:sub_8001FD4:agbcc`, `synthetic:armkeep:agbcc`, `synthetic:bgfixed:agbcc`,
// `synthetic:entrypair:mwcc_242_81`.
//
// `constant-arms` does NOT protect `maxi`/`mini`/`absdiff`/`clamp0`/`bittest`, a claim this header
// used to make and which is false: agbcc emits those as a branch-over-one-instruction (or
// branchlessly), so their targets hold no two-armed diamond and `two-arms-one-head` has already
// refused them. Ablating `constant-arms` leaves all five byte-exact and unmoved. A control that
// stays MATCH because it never reaches the table is not evidence the table is right — the shapes
// that DO reach it are the bodied ones above, and they are what `packages/cli/test/matching` pins.
//
...
// asserts the opposite for its own shape, and the assertion is what `carried.every(const)` plus
// `bare-arms` IS: on the axis those two clauses read, the mapping is a function, the differ never
// has to referee it, and the fan does not grow (4 candidates before, 4 after). An axis where a
// default belongs doubles every enumeration to referee a question with one answer.
//
...
export const SELECT_GATES: readonly Gate<SelectCandidate>[] = [
  {
    ...
    // it (`sw2`, packages/cli/test/matching — a bodied one, so `bare-arms` refuses it).
  ...
  },
  {
    ...
    guardedBy: 'retsink.test.ts: every constant-arm clause refuses a shape the two-armed evidence does not cover',
  ...
  },
  {
    ...
    //
    // ON ITS OWN, AND UNREACHED. Ablating it moves no corpus row and no fixture in this tree, and
    // that is not the same as being redundant — it is the only clause between an operand-less `ret`
    // and admission, because `carried` is then `[]` and `[].every(…)` is TRUE, so `constant-arms`
    // would admit it. What is missing is an INHABITANT: `VOID_SELECT` in retsink.test.ts is not one
    // (the Thumb frontend hands even a void function `ret r0`, so its merge is refused one check
    // earlier, on `ret.operands.every(o => m.params.includes(o))` — measured by running the whole
    // table ablated to `[]`, which still does not sink it). The shape that WOULD reach it is a
    // frontend emitting `ret` with no operands at all — `frontend/thumb.ts`'s `viaReturnReg` branch,
    // and `retOps` in the MIPS and PPC frontends. Until one of those is pinned here, treat this
    // clause as an argued guard and not as a measured one.
  ...
  },
  {
    ...
    sound: false,
    // SUBSUMED ON THE CORPUS by `bare-arms` (header). Its fixture is the one shape only it decides:
    // BARE arms carrying non-constants, the computation hoisted into the head.
    guardedBy: 'retsink.test.ts: every constant-arm clause refuses a shape the two-armed evidence does not cover',
    rejects: (c) => !c.carried.every((o) => o?.opcode === 'const'),
  },
  {
    // THE OTHER HALF OF THE HOIST, and the one this admission shipped without. The constant can
    // only rise above the compare if the arm holds nothing else; give an arm a body and the
    // merge-variable spelling keeps its diamond too, so sinking trades a byte-exact match for the
    // same shape in the other ARM ORDER — which only the ranked `/flip-branch` can pay back, and
    // the unranked primary cannot. Paid for by five measured byte-exact matches (header).
    id: 'bare-arms',
    why: 'an arm with a body pins the constant below the compare, so the merge variable keeps the diamond too',
    sound: false,
    // Also the clause the compiled evidence is committed FOR: `selbody` keeps its diamond in both
    // spellings (select-spelling.test.ts), and five shapes lose a byte-exact match without it
    // (packages/cli/test/matching). On the corpus, dropping it changes `pokeemerald:GiveBerryPowder`.
    guardedBy: 'select-spelling.test.ts: selbody keeps its diamond in both spellings',
    rejects: (c) => c.armBodies.some((ops) => ops.length > 0),
  },
];
...
export function sinkReturns(
  ...
  for (const m of [...fn.blocks]) {
    ...
    /** What an arm carries in, and what it computes BESIDES that. A BARE arm's only ops are the
     *  `const`s it hands the merge and its own `br`; anything else is a body, and a body is what
     *  pins the constant below the compare (`bare-arms`). */
    const armIn = (p: Block) => {
      const args = p.ops[p.ops.length - 1].successors[0].args;
      const carried = ret.operands.map((o) => defs.get(args[m.params.indexOf(o)]));
      const bound = new Set(carried.filter((o) => o?.opcode === 'const'));
      return { carried, body: p.ops.slice(0, -1).filter((o) => !bound.has(o)) };
    };
