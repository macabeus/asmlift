...
export const PRE_RECOVERY_PASSES: PreRecoveryPass[] = [
  ...
  // would otherwise have to reason around, and it can only shrink the value graph.
  {
    ...
    // Numbering alone costs kleod:DrawLevelHud_DreamStones its match, so the pair is the atomic
    ...
    // kleod:sub_0803F68C and kleod:WorldMapScreenCheckNewWorldUnlocked, neither of them near matching.
    run: (fn) => {
