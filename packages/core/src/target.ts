...
export const MIPS_GCC: TargetDescription = {
  ...
  compilerBehaviors: {
    ...
    // the two tests. RE-MEASURED once branch-likely lifted: both spellings now get all the way
    // through on both toolchains — the `switch` one recovers a `switch`, the ladder comes back an
    // `if` nest — so what keeps it out of the fixture set is no longer a decline. It is that the
    // cross-jumped arms leave no STORE to read the split off: both spellings put their one `sw`
    // after both tests, and the interleaved instruction is the ladder's `li v0,1`. The committed
    // pair keeps its distinct-store arms for that reason. (A THREE-case body says the
