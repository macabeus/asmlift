...
export interface TargetDescription {
  ...
  compilerBehaviors: {
    ...
    // only after it. COMMITTED, not described: `corpus/agbcc-sw{frontload,ladder}.s` from
    // `corpus/probe-agbcc-sw{frontload,ladder}.c`, regenerated by
    // `scripts/regen-switch-spelling-probes.ts`, asserted in switch-arms.test.ts. (It said
    // "79 bytes" until wave 2 compiled it: 79 is odd, and no Thumb function is an odd byte count.)
    //
