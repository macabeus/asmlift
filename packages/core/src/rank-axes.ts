...
export const STRUCTURING_AXES: readonly StructuringAxis[] = [
  ...
  // FUNCTION, so a function whose `if`s were written in opposite senses reaches neither spelling: over
  ...
  // first (`synthetic:ifand_far`, whose score cannot referee anything: it MATCHES on `/flip-join`
  // either way, so its spelling is pinned by test/site-sense.test.ts instead).
  //
  ...
  // (`synthetic:chainsense`, 4/44 on the one-stamp reading and MATCH on the two), and so is the
  // long branch, which is how the premise's one counterexample stops mattering.
  //
  ...
  // one to ride past.
  {
    ...
    // BOTH stamps, which is the predicate the consumer admits a site on (structure.ts's
    // `senseFromFoldEvidence` site default): one contract, not two spellings of it 4000 lines apart.
    // The fold writes the pair in one object literal, so this is the same set of functions today.
    variantGate: (fn) =>
      fn.blocks.some((b) =>
        b.ops.some(
          (op) => typeof op.attrs.scSharedOnFall === 'boolean' && typeof op.attrs.scSharedIsTaken === 'boolean',
        ),
      ),
    strip: false,
  },
