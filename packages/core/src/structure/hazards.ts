...
export function makeLoopHazards(deps: LoopHazardDeps): LoopHazards {
  ...
  const loopEscapeHazard = (
    ...
    // pre-update value crosses the exit as an edge ARG; a value a body OP computed is named at its
    // def where the code past the latch's exit reads it (`escapesAheadOfUpdate`, analysis.ts, which
    // lists where it refuses); read from the header PARAM instead it arrives here, with neither an
    // exit slot to sink nor an op to name. All three spellings, and the agbcc listing, are in
