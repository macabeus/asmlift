...
export function makeLoopHazards(deps: LoopHazardDeps): LoopHazards {
  ...
  // re-guards it at the new point — and stays exempt here for the same reason, the one cell for
  // which re-guarding is not merely a spelling having declined before any loop is looked at
  // (structure.ts, `volatileGuardedRead`).
  //
