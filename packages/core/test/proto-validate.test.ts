...
// What each refusal is FOR: `declaredArgLayout` reads a malformed `params` as an omitted one and
// falls back to the arg-register heuristic, so anything accepted here decompiles at a guessed arity.
import { describe, expect, test } from 'vitest';

import { declaredArgLayout, validatePrototypes } from '../src/proto';
...
describe('refuses what would otherwise decompile at a guessed arity', () => {
  test('a stringly-typed count — the case declaredArgLayout silently drops', () => {
    expect(declaredArgLayout({ params: '2' } as never)).toBeUndefined();
