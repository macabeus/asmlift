...
test('P2 recovers a merged-return loop with an in-body early `return` (return-trampoline)', () => {
  ...
  expect(src).toContain('u32 v1;'); // the counter's unsigned-only evidence reconciles its declaration
  expect(src).toContain('if (v1 > 31) {'); // the bound exit becomes an in-body early return — icmp_ugt compares unsigned through the u32 declaration, no cast needed
