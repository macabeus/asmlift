...
// The two frozen witnesses of an inner loop clobbering its enclosing loop's carrier — the only ones
// in 1..7000 — live in `loop-escape-witnesses.ts`: `nested-carrier.test.ts` replays them against
// the naming walk's own nested-carrier rule, which reaches the same collision by another door.
