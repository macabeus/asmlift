...
//
// SIX `continue`s in the do-while recognizer decide which loops it will judge at all — not three,
// which is what this header claimed while it was prose with nothing linking it to the code. The
// count is now asserted against the source below, so a seventh cannot be added without landing
// here; the same assertion is what caught the miscount. Each is named with what ablating it costs,
// re-measured for this file by replacing its `continue;` with a no-op and running the whole of
// `packages/core/test` (2,586 tests):
//
//   1. guarded self-loop  `nl.selfLoop && loops.has(h)`         1 failed — THIS FILE  (GUARDED_SELFLOOP/GSL2)
//   2. multi-latch        `!nl.selfLoop && latches.length !== 1` 1 failed — THIS FILE  (MULTILATCH)
//   3. overlapping inner  a nested header whose body escapes ours  2,586 PASS — UNWITNESSED
//   4. irreducible        `!reducible`                             2,586 PASS — UNWITNESSED
...
//
// So TWO of the six are unwitnessed, not one. #4 is this file's long-standing debt, instrumented
// below. #3 is NEW debt this inventory surfaced and nothing here has chased it yet: all that is
// measured is the ablation — the whole core suite is green without it — which says no existing test
// distinguishes it and says nothing about whether a fixture could. Naming both is the point: an
// unwitnessed refusal that nobody has written down reads exactly like a witnessed one, which is how
// these went unpinned for months.
//
...
//
// EVERY TEST BELOW IS ABLATED AGAINST THE REFUSAL IT NAMES, and the inventory above says which of
// the six that leaves unwitnessed. A test that reads as a pin and passes with its rule deleted is
// worse than no test, so the status is per refusal and it is a measurement:
...
//                    this file reddens.
//   • guarded self-loop — WITNESSED TWICE (`GUARDED_SELFLOOP`, `GSL2`), and the witness took a
//                    second predecessor to find. The one-entry `SELFLOOP` below does NOT reach it:
//                    that shape is spelled as a top-tested `for` with the refusal present AND
//                    ablated, which is why its test is named for the emitter and not for the
//                    refusal. Entered from two predecessors, the same loop declines loud with the
//                    refusal and structures without it.
//
//                    THE DECLINE AND THE CORRECTNESS CLAIM ARE PINNED BY DIFFERENT FIXTURES, and
//                    conflating them is a mistake this file made once: `GUARDED_SELFLOOP` pins the
//                    refusal and CANNOT say anything about the widening, because its loop never
//                    terminates (every path into `^bb3` establishes `%0 < %1`, which is the latch
//                    test, re-read from the same two entry params). Measured: 0 of 64 seeds reach
//                    `f0` at all, 21 cap, and the "43 judged, 0 differing" the ablation reports is
//                    43 seeds that SKIP the loop. `GSL2` is the terminating version — same refusal,
//                    a latch test over a value the body increments — and it is the one that measures
//                    the widening: 0 capped, 44 of 64 seeds actually running the loop, and with the
//                    refusal ablated 64 of 64 judged, 0 differing.
...
//                    pre-existing wrong answer at depth 1 (seed 327), not fixed here, which is why
//                    the widening is not the thing being shipped. What IS shipped is the decline,
//                    pinned, so widening it is a deliberate act.
 ...
 *  is worth recording, because it was found by compiling and not by reading — two PLAIN `br` entries
 *  do not reach the refusal at all, structuring as an ordinary do-while with it present and ablated.
 *
...
// THE INVENTORY, LINKED TO THE CODE. The header above lists six refusals; nothing but this stops it
// drifting the way it already had (it said three). Counting `continue;` in the recognizer's own
// region rather than pattern-matching a line, so the assertion survives reformatting — the same
// shape `locals-written.test.ts` uses over `rank.ts`. Eight: the six shape refusals plus the two
