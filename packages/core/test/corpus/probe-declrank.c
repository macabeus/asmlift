 ...
 *
 * THIS IS THE FORWARD HALF; `probe-declrank-rev.c` is the identical body with the DECLARATION
 * LIST reversed, which is what separates declaration rank from the order of the assignments.
 *
 * Compiled by `scripts/regen-declrank-probes.ts` for ido7.1, gcc2.7.2kmc and gcc2.7.2 — flags and
 * objdump command are in each generated `*-declrank.txt` header, taken from
