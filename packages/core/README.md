...
| `rank.ts`                                 | Pure candidate enumeration + `rankBy` (an injected score function ranks). Every driver ranks through `rankBy`: the sync driver (@asmlift/cli's `decompileRanked`, which the benchmark and `bench fan` run) scores inside it, and the two async drivers (the CLI's pooled ranker, the playground's wasm scorer) compile on their own schedule, memoize the outcomes and hand `rankBy` the memo                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `stillborn.ts`, `compiler-diagnostics.ts` | The stillborn-fan rule, one copy for every driver: when the default candidate is rejected, one probe per variation name is compiled, and only a fan whose default and every probe fail with the SAME error key is left uncompiled — the rest reported as NOT COMPILED, never as dropped. `compiler-diagnostics.ts` is that key: `CompilerRejection` (a compile that ran to completion and said no, as distinct from a transient), the error lines of each compiler's output with positions normalised away, and `errorsFirst` for whatever bounds a diagnostic downstream                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
