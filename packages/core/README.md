...
Three ISA frontends (ARMv4T/Thumb, MIPS, PowerPC), five compiler families over seven target keys
(agbcc; IDO; KMC GCC; GCC 2.7.2; CodeWarrior at `mwcc_233_163n`, `mwcc_242_81` and
`mwcc_247_107`), three language backends over one neutral AST — all scored across the package seam
...
through its `R_PPC_EMB_SDA21` relocation, a call's outgoing stack arguments where the declaration
and the code agree, and a MIPS likely branch's nullified delay slot. Still DECLINED (loud, never
...
capability gap behind the mwcc switch stubs), computed tail calls, PIC and the `gp`-relative access no relocation
